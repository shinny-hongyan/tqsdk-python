    ...
    **注意** ：如果未订阅 quote，模拟交易在下单时会自动为此合约订阅 quote ，根据回测时 quote 的更新规则，如果此合约没有订阅K线或K线周期大于分钟线 **则会自动订阅一个分钟线** 。
    ...
    def __init__(self, start_dt: Union[date, datetime], end_dt: Union[date, datetime]) -> None:
        ...
        if isinstance(start_dt, datetime):
            ...
            self._start_dt = TqDatetime._get_trading_day_start_time(int(datetime(start_dt.year, start_dt.month, start_dt.day).timestamp()) * 1000000000)
        else:
        ...
        if isinstance(end_dt, datetime):
            ...
            self._end_dt = TqDatetime._get_trading_day_end_time(int(datetime(end_dt.year, end_dt.month, end_dt.day).timestamp()) * 1000000000)
        else:
    ...
    async def _gen_serial(self, ins, dur):
        ...
        chart_info = {
            "aid": "set_chart",
            "chart_id": _generate_uuid(self._api.RD, "PYSDK_backtest"),
            "ins_list": ins,
            "duration": dur,
                ...
                async for _ in update_chan:
                    ...
                    while True:
                        ...
                        if dur == 0:
                            ...
                            timestamp = item[
                                "datetime"] if dur < 86400000000000 else TqDatetime._get_trading_day_start_time(item["datetime"])
                            ...
                            timestamp = item[
                                            "datetime"] + dur - 1000 if dur < 86400000000000 else TqDatetime._get_trading_day_end_time(
                                item["datetime"]) - 999
                            if timestamp > self._end_dt:  # 超过结束时间
                                return
